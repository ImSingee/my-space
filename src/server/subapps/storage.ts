/** Server-only: per-subapp blob storage backed by the local filesystem. */
import {
  mkdir,
  readFile,
  readdir,
...
import { subappStorageDir } from '~agent/paths';
 ...
 * subapp's storage root. Rejects absolute paths and parent traversal.
 */
...
function resolvePaths(id: string, key: string) {
  const root = subappStorageDir(id);
...
/** Remove all stored blobs for a subapp (used on delete). */
export async function dropStorage(id: string): Promise<void> {
  await rm(subappStorageDir(id), { recursive: true, force: true });
}
