/** Server-only: compile a subapp source tree into deployable artifacts. */
import { spawn } from 'node:child_process';
import { promises as fs } from 'node:fs';
import path from 'node:path';
import * as esbuild from 'esbuild';
import { REPO_ROOT, subappBuildDir, subappSrcDir } from '~agent/paths';
import {
...
function browserDefine(id: string, name: string): Record<string, string> {
  return {
    ...
    __SUBAPP_NAME__: JSON.stringify(name),
    'process.env.NODE_ENV': '"production"',
  };
}

export async function buildSubapp(id: string): Promise<BuildResult> {
  const src = subappSrcDir(id);
  const out = subappBuildDir(id);
  const logs: string[] = [];

  if (!(await pathExists(src))) {
    throw new Error(`Subapp source not found: subapps/${id}`);
  }
  ...
  // 1) Connect codegen from proto (if the subapp has a backend RPC service).
