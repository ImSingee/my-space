...
import { REPO_ROOT, appBuildDir, appSrcDir } from '~agent/paths';
import {
...
export async function buildApp(id: string): Promise<BuildResult> {
  const src = appSrcDir(id);
  const out = appBuildDir(id);
  const logs: string[] = [];

  if (!(await pathExists(src))) {
    throw new Error(`App source not found: apps/${id}`);
  }
