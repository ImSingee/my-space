 ...
 *
 *   GET    /api/app/<id>/kv        → { items: KvRecord[] }   (list)
 *   GET    /api/app/<id>/kv/<key>  → KvRecord | 404          (read)
 *   PUT    /api/app/<id>/kv/<key>  → KvRecord                (upsert; body {value, secret?})
 *   DELETE /api/app/<id>/kv/<key>  → { ok: boolean }         (delete)
 *
...
function parse(request: Request): { id: string; key: string } | null {
  ...
  const match = parsed?.rest.match(/^\/kv(?:\/(.*))?$/);
  if (!parsed || !match) return null;
  return { id: parsed.id, key: decodeURIComponent(match[1] ?? '') };
}
