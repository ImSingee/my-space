...
function parse(request: Request): { id: string; key: string } | null {
  ...
  const match = url.pathname.match(/^\/api\/subapps\/([^/]+)\/storage\/?(.*)$/);
...
async function handle({ request }: { request: Request }): Promise<Response> {
  ...
  const storage = await import('~server/subapps/storage');
  try {
...
export const Route = createFileRoute('/api/subapps/$subappId/storage/$')({
  server: {
    handlers: {
      GET: handle,
      PUT: handle,
