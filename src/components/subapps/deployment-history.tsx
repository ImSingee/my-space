...
import { deploymentsQueryOptions } from '~queries/subapps';
import type { DeploymentSummary } from '~server/subapps/manage';
import { rollbackSubappFn } from '~server/subapps';
...
export function DeploymentHistory({ subappId }: { subappId: string }) {
  const router = useRouter();
  const qc = useQueryClient();
  const query = useQuery(deploymentsQueryOptions(subappId));

  const rollback = useMutation({
    mutationFn: (deploymentId: string) =>
      rollbackSubappFn({ data: { id: subappId, deploymentId } }),
    onSuccess: (result) => {
      toast.success(`Restored v${result.version}`);
      void qc.invalidateQueries(deploymentsQueryOptions(subappId));
    ...
    <Card withBorder padding="lg">
      ...
      {query.isLoading ? (
        ...
        <Text size="sm" c="dimmed">
          No deployments yet. Deploy this subapp to create the first version.
        </Text>
      ) : (
        <Stack gap="xs">
