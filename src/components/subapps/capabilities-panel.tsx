...
import { subappOpsQueryOptions } from '~queries/subapps';
import { deleteStorageObjectFn, runCronJobFn } from '~server/subapps';
...
export function CapabilitiesPanel({ subappId }: { subappId: string }) {
  const qc = useQueryClient();
  const query = useQuery(subappOpsQueryOptions(subappId));

  const runCron = useMutation({
    mutationFn: (name: string) =>
      runCronJobFn({ data: { id: subappId, name } }),
    onSuccess: (res, name) => {
      ...
      void qc.invalidateQueries(subappOpsQueryOptions(subappId));
    },
  ...
  const deleteObject = useMutation({
    mutationFn: (key: string) =>
      deleteStorageObjectFn({ data: { id: subappId, key } }),
    onSuccess: () => {
    ...
    <Card withBorder padding="lg">
      ...
      {!anyEnabled ? (
        <Text size="sm" c="dimmed">
          No extended capabilities are enabled for this subapp.
        </Text>
      ) : (
        <Stack gap="lg">
          {ops.backend.capable ? (
