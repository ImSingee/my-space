...
| `.env.example`          | Change the sample `DATABASE_URL` database name from `project` to the real local database name. Change `APP_URL` to the project's public origin. | This keeps copied environment files aligned with the initialized project.                          |
