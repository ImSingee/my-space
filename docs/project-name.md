...
| `.env.example`          | Change the sample `DATABASE_URL` database name from `project` to the real local database name.                                                | This keeps copied environment files aligned with the initialized project.                          |
