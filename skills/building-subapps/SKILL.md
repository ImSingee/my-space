---
name: building-subapps
description: How to design, build, and deploy a Hatch subapp end to end — manifest, Connect proto, Deno Connect backend, React SPA with hash routing, dashboard widgets, and a per-subapp Postgres database. Use this whenever you create or modify a subapp.
---

# Building a Hatch subapp

A subapp is an independent app the platform builds and runs for you. You only
edit source under `subapps/<id>/`; the platform handles codegen, bundling,
...
subapps/<id>/
  ...
  proto/service.proto    Connect RPC service (one service per subapp)
...
`gen/service_pb.ts` is generated by `deploy_subapp` (it runs `buf generate`).
...
```json
{
  ...
  "rpc": { "proto": "proto/service.proto", "service": "subapp.v1.TodoService" },
...
Use `query_subapp_db` to create tables and inspect data while developing. The
backend should also create its tables on startup (idempotent `create table if
not exists`). Each subapp has its OWN database — no cross-subapp access.
...
on the subapp page). Verified requests are forwarded to your backend under
...
HTTP API: `GET/PUT/DELETE /api/subapps/<id>/storage/<key>`, and
`GET /api/subapps/<id>/storage/` returns a JSON list of objects.
...
1. `create_subapp` to scaffold.
2. Read the files, then edit proto → backend → app → widgets, keeping the
   manifest in sync.
3. `deploy_subapp` to build and go live. On failure, read the build output, fix
   the source, and deploy again.
4. Tell the user what you built and that they can open it from the Subapps page
